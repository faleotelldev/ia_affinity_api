...
class AnalysisService:
    ...
    def calculate_affinity(db: Session, candidate_id: int, job_id: int) -> dict:
        ...
        # Procesar skills de la descripción del trabajo
        job_skills = []
        if job.description:
            job_skills = [
                s.strip().lower()
   ...
   # app/services/analysis_service.py
    ...
    @staticmethod
    def analyze_and_save_by_ids(db, candidate_id: int, job_id: int):
        svc = AnalysisService(use_deepseek_score=True)
        result = svc.analyze_by_ids(db, candidate_id, job_id)
        ...
        saved = AnalysisRepository.save_analysis(
            ...
            candidate_id,
            job_id,
            affinity_score=result.get("score", 0),
            match_reason=", ".join(result.get("matching_skills", [])),
            features_json={
                "candidate_skills_processed": result.get("candidate_skills_processed", []),
                "job_skills_processed": result.get("job_skills_processed", [])
            }
        )
